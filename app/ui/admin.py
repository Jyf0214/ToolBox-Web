    ...
    async def admin_page(request: Request):
                    ...
                    with ui.column().classes("p-6 w-full"):
                        ...
                        with ui.column().classes("w-full gap-4"):
                            ...
                            async def request_code():
                                ...
                                now_req = time.time()
                                ip_info = reset_data.get(
                                    client_ip, {"last_request": 0}
                                )
                            ...
                            async def dangerous_reset_db():
                                try:
                                    print(
                                        f"CRITICAL: IP {client_ip} 正在重置整个数据库！"
                                    )
                            ...
                            verify_btn = ui.button(
                                "确认验证", on_click=verify_step
                            ).classes("w-full hidden shadow-md")
                            ...
                            with ui.row().classes("w-full justify-end mt-4"):
                                ui.button("取消", on_click=reset_dialog.close).props(
                                    "flat"
                                )
            return
        ...
        with ui.left_drawer(value=True).classes("bg-slate-50 border-r") as left_drawer:
            with ui.column().classes("w-full p-4 gap-2"):
                def nav_item(label, icon, target_section):
                    def switch_section():
                        for sect in sections.values():
                            sect.set_visibility(False)
                        sections[target_section].set_visibility(True)
                        if ui.query(".nav-active"):
                            ui.query(".nav-active").classes(remove="bg-primary text-white nav-active")
                            ui.query(".nav-active").classes(add="text-slate-600")
                        # Note: NiceGUI doesn't easily support dynamic class changing on individual elements in this loop 
                        # without more state management. We'll use a simpler approach for now.
                        ui.notify(f"切换到 {label}", type='info', position='bottom-right')

                    return ui.button(label, icon=icon, on_click=switch_section).props(
                        "flat align=left"
                    ).classes("w-full text-slate-600 rounded-lg py-2")
        ...
        with ui.column().classes("p-4 sm:p-8 w-full max-w-5xl mx-auto"):
            ...
            with ui.column().classes("w-full hidden") as sections["tools"]:
                with ui.row().classes("w-full items-center justify-between mb-6"):
                    ...
                    ui.button("刷新列表", icon="refresh", on_click=handle_refresh).props("outline size=sm")
